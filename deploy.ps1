...
Write-Host "0. Building Frontend Locally..." -ForegroundColor Cyan
if (-not (Get-Command npm -ErrorAction SilentlyContinue)) {
    Write-Error "npm is required for local build. Please install Node.js."
    exit 1
}

Push-Location frontend
$env:REACT_APP_API_URL = "http://$PublicIP:8000"
...
$RemoteCommands = "
    ...
    sudo rm -rf app
    mkdir -p app
    sudo tar -xzf deploy.tar.gz -C app
